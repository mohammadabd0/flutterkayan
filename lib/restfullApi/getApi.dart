...
List<Book> bookList = items.map((item) {
  ...
  String author = bookData.containsKey("authors") ? bookData["authors"][0] : "Author is not found";

  String publishedDate = bookData.containsKey("publishedDate") ? bookData["publishedDate"].toString() : DateTime.now().toString();
  DateTime? dateTime;
  ...
  if (publishedDate.length == 4 && int.tryParse(publishedDate) != null) {
    ...
    try {
      ...
      dateTime = DateTime(parsedDateTime.year, parsedDateTime.month, parsedDateTime.day);
    } catch (e) {
